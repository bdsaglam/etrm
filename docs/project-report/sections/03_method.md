...
Our key design principle is to preserve TRM's decoder architecture unchanged. The recursive reasoning mechanism and deep supervision that make TRM effective are retained; we only modify how task context is obtained.
...
The network $f_\theta$ is a small 2-layer transformer. Despite its simplicity, the recursive application creates an effective depth of $H \times (L+1) \times 2 = 42$ layers.

### 3.2.3 Deep Supervision with Adaptive Computation

TRM uses deep supervision: the carry state $(\mathbf{y}, \mathbf{z})$ persists across training steps, with each step providing supervision. A Q-head learns when to halt, implementing Adaptive Computation Time (ACT). During training, an exploration probability $p_{\text{explore}}$ encourages the model to sometimes continue beyond the Q-head's recommendation.
...
**Stage 1: Per-Demo Grid Encoding.** Each demonstration pair $(\mathbf{x}_i^{\text{in}}, \mathbf{x}_i^{\text{out}})$ is encoded independently. We concatenate the input and output grids and process them with a transformer:
...
**Stage 2: Set Aggregation via Cross-Attention.** To aggregate the $K$ demo encodings $\{\mathbf{e}_i\}_{i=1}^K$ into the final context, we use cross-attention with learnable query tokens:
...
3. Pool the context and project to variational parameters:
   ...
   $$
4. Sample via reparameterization:
   $$
...
The KL divergence regularizes toward a standard normal prior:
$$
\mathcal{L}_{\text{KL}} = D_{\text{KL}}(q(\mathbf{z}|\text{demos}) \| p(\mathbf{z})) = -\frac{1}{2}\sum_j \left(1 + \log \sigma_j^2 - \mu_j^2 - \sigma_j^2\right)
$$

**Per-Demo VAE.** Inspired by the Latent Program Network (LPN) architecture, this variant applies variational inference to each demonstration independently before aggregation:
...
2. Project each encoding to per-demo variational parameters:
   ...
   $$
3. Sample each demo's latent: $\mathbf{z}_i = \boldsymbol{\mu}_i + \boldsymbol{\sigma}_i \odot \boldsymbol{\epsilon}_i$
...
The KL loss averages over valid demonstrations:
$$
...
**Note on LPN**: We adopt LPN's encoder architecture for comparison, but we do *not* use their gradient-based test-time search. Our encoders produce fixed representations without test-time optimization.
...
**Hierarchical Refinement**: Each ACT step, the encoder refines its states using the same H/L loop pattern as the decoder:
...
**Joint Evolution**: Unlike feedforward encoders that compute context once, the iterative encoder's carry state persists across ACT steps. The encoder refines its understanding of the transformation rule as the decoder refines its prediction—a form of joint reasoning.
...
The decoder architecture remains unchanged from original TRM:
