"""
Training script for ETRMTRM - Recurrent Encoder + TRM Decoder.

This script trains TRM with a recurrent encoder that has carry state:
- Encoder called every ACT step with carry
- Context evolves across steps
- Decoder same as ETRM

Model: etrmtrm.py (TRMWithRecurrentEncoder)
Uses same training dynamics as ETRM.
"""
...
def create_model_encoder(
    ...
    """Create TRMWithRecurrentEncoder model with optimizer(s).
...
def load_pretrained_decoder(model: nn.Module, checkpoint_path: str, rank: int = 0):
    """Load pretrained TRM decoder weights into TRMWithRecurrentEncoder.inner.
...
def set_decoder_frozen(model: nn.Module, frozen: bool) -> int:
    ...
    Args:
        model: TRMWithRecurrentEncoder model (possibly compiled)
...
@hydra.main(config_path="config", config_name="cfg_pretrain_etrmtrm_arc_agi_1", version_base=None)
